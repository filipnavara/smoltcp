...
impl<'a> TryFrom<NdiscRouteInformation> for RouteInformationList<'a> {
    ...
    fn try_from(route_info: NdiscRouteInformation) -> core::result::Result<Self, Self::Error> {
        // A non-canonical prefix would be masked on emit and so would not
        // survive a round trip. Reject it instead of silently rewriting it.
        if !route_info.is_valid_route_info() || !route_info.has_canonical_prefix() {
...
impl<'a> TryFrom<&'a [NdiscRouteInformation]> for RouteInformationList<'a> {
    ...
    fn try_from(
        ...
        for (index, entry) in route_info.iter().enumerate() {
            if !entry.is_valid_route_info() || !entry.has_canonical_prefix() {
                return Err(*entry);
            }
            if route_info[..index]
                .iter()
...
mod test {
    ...
    fn test_route_information_list_validation() {
        let base = NdiscRouteInformation {
            ...
            preference: NdiscRoutePreference::Low,
            route_lifetime: Duration::from_secs(900),
            prefix: Ipv6Address::new(0x2001, 0xdb8, 1, 0xa800, 0, 0, 0, 0),
        };
        ...
        // A prefix with bits set beyond the prefix length would be masked on
        // emit, so it cannot round trip and is rejected up front.
        let non_canonical = NdiscRouteInformation {
            prefix: Ipv6Address::new(0x2001, 0xdb8, 1, 0xafff, 0, 0, 0, 2),
            ..base
        };
        assert_eq!(
            RouteInformationList::try_from(non_canonical),
            Err(non_canonical)
        );
